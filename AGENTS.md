...
- `shared/security-and-speed.md` — the security/speed rules injected into every generated `AGENTS.md` via `{{RULES}}`

## Rules

- Every generated project must get security and speed guidance. Universal rules live only in `shared/security-and-speed.md`; stack-specific rules go in that template's `AGENTS.md` under "This stack: security and speed". Do not copy the universal rules into templates.
- Each template's `AGENTS.md` is the single source of project instructions and must contain `{{RULES}}`. `CLAUDE.md` is just `@AGENTS.md` (from `common/`); do not add per-template `CLAUDE.md` files.
...
- Every template ships: README.md, AGENTS.md, CI workflow, `.github/dependabot.yml`, `.gitignore` (where the stack needs one).
...
- Follow `shared/security-and-speed.md` (imported by `CLAUDE.md`).
