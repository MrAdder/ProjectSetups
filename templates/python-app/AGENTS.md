...
- CI runs `pip-audit` on every push and weekly. The Docker image runs as the non-root `app` user; keep it that way.
...
- Log with the `logging` module (levels, `%s`-style arguments), never `print`, and do not log personal data or secrets.
